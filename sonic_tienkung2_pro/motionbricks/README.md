...
NVIDIA-origin source code is licensed under **Apache 2.0**, except where a
component notice states otherwise. Pretrained model weights are licensed under the
**NVIDIA Open Model License**. See the repository `LICENSE` and `NOTICE` files.
...
For questions about this downstream package, use the support or issue channel
supplied by its distributor. NVIDIA upstream contacts are not responsible for
downstream TienKung2 Pro modifications.
