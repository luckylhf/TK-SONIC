...
class ManagerEnvWrapper:
    ...
    def begin_seq_motion_samples(self, global_rank=0):
        ...
        # Apply filter/exclude patterns from motion_lib_cfg before eval
        if self.motion_command is not None:
            motion_lib_cfg = getattr(self.motion_command.cfg, "motion_lib_cfg", None)
            if motion_lib_cfg is not None:
                filter_patterns = motion_lib_cfg.get("filter_patterns", []) or []
                exclude_patterns = motion_lib_cfg.get("exclude_patterns", []) or []
                if filter_patterns or exclude_patterns:
                    n = self._motion_lib.apply_filter(filter_patterns, exclude_patterns=exclude_patterns)
                    logger.info(f"[Eval] Applied filter: {n} motions active (filter={filter_patterns}, exclude={exclude_patterns})")
