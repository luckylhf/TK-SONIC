...
def exceeded_body_pos(
    ...
    result = pos_diff.norm(dim=-1).gt(threshold).any(dim=-1)

    # --- DEBUG: one-shot print on first call ---
    if not hasattr(exceeded_body_pos, "_debug_done"):
        exceeded_body_pos._debug_done = True
        print(f"\n[DEBUG exceeded_body_pos] threshold={threshold}")  # noqa: T201
        print(f"  tracked indices: {tracked}")  # noqa: T201
        print(f"  body_names param: {body_names}")  # noqa: T201
        print(f"  command.cfg.body_names: {command.cfg.body_names}")  # noqa: T201
        tracked_names = [command.cfg.body_names[i] for i in tracked]
        print(f"  tracked body names: {tracked_names}")  # noqa: T201
        print(f"  num_envs terminated: {result.sum().item()} / {result.shape[0]}")  # noqa: T201
        if len(tracked) > 0:
            print(f"  pos_diff norm (max over envs): {pos_diff.norm(dim=-1).max():.4f}")  # noqa: T201
            print(f"  pos_diff norm (mean over envs): {pos_diff.norm(dim=-1).mean():.4f}")  # noqa: T201
        else:
            print(f"  WARNING: No tracked bodies found!")  # noqa: T201
        if result.sum() > 0:
            first_bad = result.nonzero(as_tuple=True)[0][0].item()
            print(f"  --- First terminated env [{first_bad}] ---")  # noqa: T201
            for ti, tn in enumerate(tracked_names):
                print(f"    [{ti}] {tn}: ref=({command.body_pos_relative_w[first_bad, tracked[ti], 0]:.4f}, {command.body_pos_relative_w[first_bad, tracked[ti], 1]:.4f}, {command.body_pos_relative_w[first_bad, tracked[ti], 2]:.4f})  robot=({command.robot_body_pos_w[first_bad, tracked[ti], 0]:.4f}, {command.robot_body_pos_w[first_bad, tracked[ti], 1]:.4f}, {command.robot_body_pos_w[first_bad, tracked[ti], 2]:.4f})  dist={pos_diff[first_bad, ti].norm():.4f}")  # noqa: T201
            print(f"  ref  root pos: {command.body_pos_relative_w[first_bad, 0]}")  # noqa: T201
            print(f"  robot root pos: {command.robot_body_pos_w[first_bad, 0]}")  # noqa: T201
