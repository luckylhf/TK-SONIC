...
class TrackingCommand(CommandTerm):
    ...
    def __init__(
        ...
        self.lower_joint_isaaclab_indices = env.cfg.isaaclab_to_mujoco_mapping.get(
            "lower_joint_isaaclab_indices", list(range(12))
        )
        self.lower_joint_indices_mujoco = [
            self.isaaclab_to_mujoco_dof[i] for i in self.lower_joint_isaaclab_indices
        ]
    ...
    def create_offline(
        ...
        inst.lower_joint_isaaclab_indices = isaaclab_to_mujoco_mapping.get(
            "lower_joint_isaaclab_indices", list(range(12))
        )
        if hasattr(inst, "isaaclab_to_mujoco_dof"):
            inst.lower_joint_indices_mujoco = [
                inst.isaaclab_to_mujoco_dof[i] for i in inst.lower_joint_isaaclab_indices
            ]
    ...
    _VELOCITY_THRESHOLDS = (0.1, 0.5, 1.5)  # m/s: static / low / medium / high

    @property
    def velocity_class(self) -> torch.Tensor:
        """Reference-motion velocity class per env: 0=static, 1=low, 2=med, 3=high."""
        speed = self.anchor_lin_vel_w.norm(dim=-1)
        cls = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        cls[speed >= self._VELOCITY_THRESHOLDS[0]] = 1
        cls[speed >= self._VELOCITY_THRESHOLDS[1]] = 2
        cls[speed >= self._VELOCITY_THRESHOLDS[2]] = 3
        return cls
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # --- DEBUG: check body pos mismatch after reset ---
        if not hasattr(self, "_debug_reset_cnt"):
            self._debug_reset_cnt = 0
        if len(env_ids) > 0 and self._debug_reset_cnt < 3:
            self._debug_reset_cnt += 1
            # Read robot body positions after state write
            robot_bp = self.robot.data.body_pos_w[env_ids][:, self.body_indexes]
            ref_bp = self.body_pos_relative_w[env_ids]
            bp_err = (robot_bp - ref_bp).norm(dim=-1)
            bp_err_z = (robot_bp[..., 2] - ref_bp[..., 2]).abs()
            try:
            ...
            try:
                body_names_all = self.robot.data.body_names
            except Exception:  # noqa: BLE001, PERF203
                pass
        ...
        if not hasattr(self, "_debug_relative_cnt"):
            self._debug_relative_cnt = 0
        if self._debug_relative_cnt < 3:
            self._debug_relative_cnt += 1
            raw_body_pos = self.body_pos_w[0]
            rel_body_pos = self.body_pos_relative_w[0]
            robot_body_pos = self.robot_body_pos_w[0]
            for i, name in enumerate(self.cfg.body_names):
                ref = rel_body_pos[i]
                rob = robot_body_pos[i]
                err = (ref - rob).norm()
