# Modified for TienKung2 Pro support; Copyright 2026 luckylhf.
...
class RewardsCfg:
    ...
    # per-joint-group split rewards (tienkung2_pro fullsplit)
    tracking_joint_pos_waist = None
    tracking_joint_pos_hip = None
    tracking_joint_pos_hip_yaw = None
    tracking_joint_pos_knee = None
    tracking_joint_pos_ankle = None
    tracking_joint_pos_shoulder = None
    tracking_joint_pos_elbow = None
    tracking_joint_vel_hip = None
    tracking_joint_vel_hip_yaw = None
    tracking_joint_vel_knee = None
    tracking_joint_vel_ankle = None
    tracking_joint_vel_shoulder = None
    tracking_joint_vel_elbow = None
    tracking_joint_vel_wrist = None
    tracking_body_pos_wrist = None
    tracking_body_pos_foot = None
    anchor_planar_drift = None
    anchor_planar_drift_ungated = None
    anchor_static_planar_vel = None
    foot_contact_vel_penalty = None
...
def tracking_joint_pos_error(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    joint_indices: list[int],
) -> torch.Tensor:
    """Joint position tracking reward for a specific group of joints (exp kernel).

    Args:
        env: The environment.
        command_name: Name of the tracking command term.
        std: Standard deviation for the Gaussian kernel.
        joint_indices: IsaacLab DOF indices of the joints to track.

    Returns:
        Reward tensor of shape (num_envs,) in [0, 1].
    """
    command: TrackingCommand = env.command_manager.get_term(command_name)
    ref_pos = command.joint_pos[:, joint_indices]
    robot_pos = command.robot_joint_pos[:, joint_indices]
    err = ((ref_pos - robot_pos) ** 2).mean(dim=-1)
    return torch.exp(-err / (std * std))


def tracking_joint_vel_error(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    joint_indices: list[int],
) -> torch.Tensor:
    """Joint velocity tracking reward for a specific group of joints (exp kernel).
    ...
    """
    command: TrackingCommand = env.command_manager.get_term(command_name)
    ref_vel = command.joint_vel[:, joint_indices]
    robot_vel = command.robot_joint_vel[:, joint_indices]
    err = ((ref_vel - robot_vel) ** 2).mean(dim=-1)
    return torch.exp(-err / (std * std))
...
def anchor_planar_drift_penalty(
    ...
    """Penalize XY anchor drift when the reference is nearly static.

    Fades out during motions with meaningful reference translation so it
    doesn't fight the policy during walking. A dead-zone avoids penalising
    small stationary jitter; larger drift gets a quadratic penalty.
    Returns a positive penalty (use negative weight).
    """
    command: TrackingCommand = env.command_manager.get_term(command_name)
    ref_planar_speed = torch.linalg.vector_norm(command.anchor_lin_vel_w[:, :2], dim=-1)
    static_gate = torch.clamp(
        1.0 - ref_planar_speed / max(ref_speed_threshold, 1e-6), min=0.0, max=1.0
    )
    planar_err = torch.linalg.vector_norm(
        command.anchor_pos_w[:, :2] - command.robot_anchor_pos_w[:, :2], dim=-1
    )
    drift_excess = torch.clamp(planar_err - drift_tolerance, min=0.0)
...
def anchor_static_planar_velocity_penalty(
    ...
    """Penalize robot anchor XY velocity when the reference is nearly static.

    Acts earlier than anchor_planar_drift_penalty by discouraging the policy
    from initiating recovery steps before drift accumulates.
    Returns a positive penalty (use negative weight).
    """
    command: TrackingCommand = env.command_manager.get_term(command_name)
    ref_planar_speed = torch.linalg.vector_norm(command.anchor_lin_vel_w[:, :2], dim=-1)
    ...
    robot_planar_speed = torch.linalg.vector_norm(
...
def foot_contact_vel_penalty(
    ...
    foot_body_idx_l: int = 3,
    foot_body_idx_r: int = 6,
) -> torch.Tensor:
    """参考动作脚接触地面时，惩罚机器人脚部线速度，抑制不必要的踏步。

    Returns a positive penalty (use negative weight).
    """
    command: TrackingCommand = env.command_manager.get_term(command_name)
    contact_l = command.feet_l[:, 0]  # (num_envs,)
    contact_r = command.feet_r[:, 0]
    vel_l = command.robot_body_lin_vel_w[:, foot_body_idx_l, :]  # (num_envs, 3)
    vel_r = command.robot_body_lin_vel_w[:, foot_body_idx_r, :]
    speed_l = torch.linalg.vector_norm(vel_l, dim=-1)
    speed_r = torch.linalg.vector_norm(vel_r, dim=-1)
    excess_l = torch.clamp(speed_l - vel_tolerance, min=0.0)
    excess_r = torch.clamp(speed_r - vel_tolerance, min=0.0)
    return (torch.square(excess_l) * contact_l + torch.square(excess_r) * contact_r) * 0.5
