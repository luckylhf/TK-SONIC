...
class TRLPPOTrainer(PPOTrainer):  # noqa: F405
    ...
    def _init_trl(
        ...
        **kwargs,
        ...
        # Only rank 0 writes to TensorBoard to avoid duplicate events files
        self._tb_writer = (
            SummaryWriter(log_dir=log_dir)
            if log_dir and self.accelerator.is_main_process
            else None
        )
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        """  # noqa: D212
        # Convert all Tensor values to Python scalars before logging
        # This prevents TensorBoard warnings from parent class
        converted_logs = {}

        # Log curriculum stage if active
        for cb in self.callback_handler.callbacks:
            if hasattr(cb, "current_stage") and hasattr(cb, "stages"):
                converted_logs["curriculum/stage"] = float(cb.current_stage)
                converted_logs["curriculum/stage_total"] = float(len(cb.stages))
                break

        for key, val in logs.items():
            if isinstance(val, torch.Tensor):
                converted_logs[key] = val.item() if val.numel() == 1 else val.mean().item()
            else:
                converted_logs[key] = val

        if self.state.epoch is not None:
            converted_logs["epoch"] = self.state.epoch
        if self.args.include_num_input_tokens_seen:
            converted_logs["num_input_tokens_seen"] = self.state.num_input_tokens_seen
            if start_time is not None:
                speed_metrics(  # noqa: F405
                    "train", start_time, num_tokens=self.state.num_input_tokens_seen
                )

        output = {**converted_logs, **{"step": self.state.global_step}}  # noqa: PIE800
        self.state.log_history.append(output)

        # Write all metrics directly to TensorBoard
        if self._tb_writer is not None:
            for key, val in converted_logs.items():
                # Values are already converted to scalars above
                if isinstance(val, (int, float)):
                    self._tb_writer.add_scalar(f"train/{key}", val, self.state.global_step)

        # Delegate to parent for callback triggers (Wandb, etc.)
        super().log(converted_logs, start_time)
