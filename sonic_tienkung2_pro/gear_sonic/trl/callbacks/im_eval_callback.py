...
class ImEvalCallback(TrainerCallback):
    ...
    def evaluate_policy(self):
        ...
        with torch.no_grad():
            ...
            if self.eval_only:
                if self.output_dir is not None and self.accelerator.is_main_process:
    ...
    def _post_eval_env_step(self, actor_state):
        ...
        if self.curr_steps >= curr_max or self.terminate_state.sum() == self.env.num_envs:
            ...
            # 每个 loop 结束后增量保存失败动作，防止 OOM 导致数据丢失
            if self.output_dir is not None and self.accelerator.is_main_process:
                failed_mask = self.terminate_state.cpu().numpy().astype(bool)
                if failed_mask.any():
                    import os as _os
                    _os.makedirs(self.output_dir, exist_ok=True)
                    failed_keys_path = _os.path.join(self.output_dir, "failed_keys_incremental.txt")
                    loop_failed_keys = self.env._motion_lib._motion_data_keys[
                        self.env.motion_ids.cpu().numpy()[failed_mask]
                    ]
                    with open(failed_keys_path, "a") as _f:
                        _f.write("\n".join(loop_failed_keys) + "\n")
            ...
            if self.env_eval_loop_idx >= self.num_total_env_eval_loops:
                ...
                # Define subsets — use names present in body_names, with fallback for different robots
                def _find_name(candidates, names):
                    for c in candidates:
                        if c in names:
                            return c
                    return None

                _hip_l = _find_name(["hip_roll_l_link", "left_hip_roll_link"], body_names)
                _hip_r = _find_name(["hip_roll_r_link", "right_hip_roll_link"], body_names)
                _knee_l = _find_name(["knee_pitch_l_link", "left_knee_link"], body_names)
                _knee_r = _find_name(["knee_pitch_r_link", "right_knee_link"], body_names)
                _ankle_l = _find_name(["ankle_roll_l_link", "left_ankle_roll_link"], body_names)
                _ankle_r = _find_name(["ankle_roll_r_link", "right_ankle_roll_link"], body_names)
                _torso = _find_name(["body_yaw_link", "torso_link"], body_names)
                _wrist_l = _find_name(["wrist_roll_l_link", "left_wrist_yaw_link"], body_names)
                _wrist_r = _find_name(["wrist_roll_r_link", "right_wrist_yaw_link"], body_names)
                _shld_l = _find_name(["shoulder_roll_l_link", "left_shoulder_roll_link"], body_names)
                _shld_r = _find_name(["shoulder_roll_r_link", "right_shoulder_roll_link"], body_names)
                _elbow_l = _find_name(["elbow_pitch_l_link", "left_elbow_link"], body_names)
                _elbow_r = _find_name(["elbow_pitch_r_link", "right_elbow_link"], body_names)

                legs_subset_names = [n for n in [_hip_l, _knee_l, _ankle_l, _hip_r, _knee_r, _ankle_r] if n]
                vr_3points_subset_names = [n for n in [_torso, _wrist_l, _wrist_r] if n]
                other_upper_bodies_subset_names = [n for n in ["pelvis", _shld_l, _elbow_l, _shld_r, _elbow_r] if n]
                foot_subset_names = [n for n in [_ankle_l, _ankle_r] if n]
                ...
                gathered_chunks = [
                    self.accelerator.gather(chunk).reshape(-1, chunk.shape[-1]) for chunk in chunks
                ]  # each with shape (num_processes * chunk_envs, D_metrics)
                all_metrics = torch.cat(gathered_chunks, dim=0)

                metric_size = all_metrics.shape[-1]
                # gather() 按进程拼接：[proc0_loop0..loopN, proc1_loop0..loopN, ...]
                # 需先 reshape 成 (n_proc, n_loops, n_envs, D)，再 permute 成 (n_loops, n_proc, n_envs, D)
                n_proc = self.accelerator.num_processes
                n_envs = self.env.num_envs
                n_loops = self.num_total_env_eval_loops
                gathered_metrics_stack = (
                    all_metrics.reshape(n_proc, n_loops, n_envs, metric_size)
                    .permute(1, 0, 2, 3)
                    .reshape(-1, metric_size)[: self.env._motion_lib._num_unique_motions]
                )  # shape: (num_unique_motions, metric_size)
                ...
                if self.accelerator.is_main_process:
                    print(f"\n{'='*60}")
                    print(f"Success Rate: {success_rate:.4f}  ({int(success_rate * self.env._motion_lib._num_unique_motions)}/{self.env._motion_lib._num_unique_motions})")
                    print(f"Progress Rate: {progress_rate:.4f}")
                    if has_obj_metrics:
                        print(
                    ...
                    print("\nAll metrics:")
                    for k, v in metrics_all_print.items():
                        print(f"  {k}: {v:.3f}")
                    print("\nSuccess metrics:")
                    for k, v in metrics_succ_print.items():
                        print(f"  {k}: {v:.3f}")
                    print(f"{'='*60}\n")

                self.success_rate = success_rate  # update progress bar with final value
            ...
            # Don't reset success_rate here — keep the last computed value visible in the progress bar
            self.curr_steps = 0
