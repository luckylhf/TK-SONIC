...
_AUX_ASSET_DIR = Path(__file__).resolve().parent


def _load_aux_asset(name):
    path = _AUX_ASSET_DIR / name
    if not path.is_file():
        raise FileNotFoundError(
            f"Required SMPL/SMPL-X auxiliary asset is not distributed: {path}. "
            "Obtain it under the applicable license and place it in this "
            "directory; see RESTORE.md section 2A."
        )
...
class SmplxLiteCoco17(SmplxLite):
    ...
    def __init__(self, **kwargs):
        ...
        # Compute mapping
        smplx2smpl = _load_aux_asset("smplx2smpl_sparse.pt")
        COCO17_regressor = _load_aux_asset("smpl_coco17_J_regressor.pt")
...
class SmplxLiteV437Coco17(SmplxLite):
    def __init__(self, **kwargs):
        ...
        # Verts437
        smplx_vids2 = _load_aux_asset("smplx_verts437.pt")
...
class SmplxLiteSmplN24(SmplxLite):
    ...
    def __init__(self, **kwargs):
        ...
        # Compute mapping
        smplx2smpl = _load_aux_asset("smplx2smpl_sparse.pt")
        smpl2joints = _load_aux_asset("smpl_neutral_J_regressor.pt")
