...
def main(override_config: omegaconf.OmegaConf):
    ...
    if simulator_type == "IsaacSim":
        ...
        extra_kit_args = env_config.config.get("extra_kit_args", "")
        if args_cli.headless:
            args_cli.kit_args = f"{base_kit_args} --no-window {extra_kit_args}".strip()
        else:
            args_cli.kit_args = f"{base_kit_args} --/renderer/activeGpu={render_gpu_idx} {extra_kit_args}".strip()
    ...
    schedule_wrapper = easydict.EasyDict(env=env, model=model, config=config.algo.config)
    ...
    for callback_name in eval_callbacks:
        if callback_name == "im_eval":
            with omegaconf.open_dict(config.callbacks.im_eval):
                ...
                if config.get("eval_output_dir", None) is not None:
                    config.callbacks.im_eval.output_dir = config.eval_output_dir
