...
try:
    ...
    _HAS_UNITREE_SDK = True
except:
    print(" ModuleNotFoundError: No module named 'unitree_sdk2py' ")
    _HAS_UNITREE_SDK = False
...
class DefaultEnv:
    ...
    def init_scene(self):
        ...
        self.torso_link_name = self.config.get("TORSO_LINK", "torso_link")
        self.torso_index = mujoco.mj_name2id(self.mj_model, mujoco.mjtObj.mjOBJ_BODY, self.torso_link_name)
        ...
        # Accept any freejoint (type 0 in MuJoCo) or the legacy name "floating_base_joint"
        joint_names = [self.mj_model.joint(i).name for i in range(self.mj_model.njnt)]
        joint_types = [self.mj_model.joint(i).type for i in range(self.mj_model.njnt)]
        self.use_floating_root_link = (
            "floating_base_joint" in joint_names
            or mujoco.mjtJoint.mjJNT_FREE in joint_types
        )
        ...
        if self.config["ENABLE_ELASTIC_BAND"] and self.use_floating_root_link:
            ...
            if "g1" in self.config["ROBOT_TYPE"]:
                ...
                self.band_attached_link = self.mj_model.body("torso_link").id
            elif "tienkung2_pro" in self.config["ROBOT_TYPE"]:
                self.band_attached_link = self.mj_model.body(self.root_body).id
            else:
        ...
        for i in range(self.mj_model.njnt):
            ...
            if any(
                [
                    part_name in name
                    for part_name in ["hip", "knee", "ankle", "waist", "shoulder", "elbow", "wrist", "body_yaw"]
                ]
            ):
    ...
    def get_head_pose(self) -> np.ndarray:
        root_pos = self.mj_data.body(self.torso_link_name).xpos.copy()
        # Reorder quaternion from MuJoCo [w,x,y,z] to scipy [x,y,z,w]
        root_quat = self.mj_data.body(self.torso_link_name).xquat.copy()[[1, 2, 3, 0]]
    ...
    def prepare_obs(self) -> Dict[str, any]:
        ...
        pose = np.zeros(13)
        torso_link = self.mj_model.body(self.torso_link_name).id
