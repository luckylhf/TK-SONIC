...
class MotionLibBase:
    def __init__(self, motion_lib_cfg, num_envs, device):
        ...
        # Save full-dataset side-data backups for curriculum apply_filter()
        self._all_smpl_data = self.smpl_data
        self._all_soma_data = self.soma_data
        self._all_object_data = self.object_data
    ...
    def load_data(self, motion_file):
        ...
        # Save full-dataset backup for curriculum learning (apply_filter uses these)
        self._all_motion_data_list = self._motion_data_list
        self._all_motion_data_keys = self._motion_data_keys
    ...
    def apply_filter(self, filter_patterns: list[str], exclude_patterns: list[str] | None = None) -> int:
        """Runtime curriculum upgrade: replace active motion set with a filtered subset.

        Filters ``_all_motion_data_list`` / ``_all_motion_data_keys`` (the full dataset
        saved at init time) using regex patterns, then resets all sampling state so the
        new subset starts fresh.  Also syncs smpl_data / soma_data / object_data so their
        per-motion indices stay aligned with the new _motion_data_keys.

        Args:
            filter_patterns: List of regex patterns (fullmatch).  Empty list = all motions.
            exclude_patterns: Optional list of regex patterns to exclude (fullmatch).

        Returns:
            Number of motions in the new active set.
        """
        if not filter_patterns:
            mask = np.ones(len(self._all_motion_data_keys), dtype=bool)
        else:
            compiled = [re.compile(p) for p in filter_patterns]
            mask = np.array(
                [any(r.fullmatch(k) for r in compiled) for k in self._all_motion_data_keys]
            )

        # Apply exclusion patterns on top
        if exclude_patterns:
            excl_compiled = [re.compile(p) for p in exclude_patterns]
            excl_mask = np.array(
                [any(r.fullmatch(k) for r in excl_compiled) for k in self._all_motion_data_keys]
            )
            mask = mask & ~excl_mask

        if mask.all():
            self._motion_data_list = self._all_motion_data_list
            self._motion_data_keys = self._all_motion_data_keys
            mask = None
        else:
            self._motion_data_list = self._all_motion_data_list[mask]
            self._motion_data_keys = self._all_motion_data_keys[mask]

        # Sync per-motion side-data lists so indices stay aligned
        for attr in ("smpl_data", "soma_data", "object_data"):
            all_attr = f"_all_{attr}"
            if hasattr(self, all_attr):
                all_data = getattr(self, all_attr)
                if all_data is None:
                    continue
                if mask is None:
                    setattr(self, attr, all_data)
                else:
                    setattr(self, attr, [all_data[i] for i, m in enumerate(mask) if m])
        ...
        if self.use_adaptive_sampling:
            self.init_adaptive_sampling()
        logger.info(f"[Curriculum] Active motions: {self._num_unique_motions}")
