...
[![License](https://img.shields.io/badge/License-Multiple-76B900.svg)](NOTICE)
...
> **SONIC v1.3 合规说明：** 本包是多许可组件集合，请先阅读 `NOTICE`、
> `MODIFICATIONS.md` 和 `legal/`。MakeHuman 派生的遥操 GUI 为 AGPL-3.0-or-later
> 组件；受限 SMPL/SMPL-X 辅助资产不随包分发，恢复方式见 `RESTORE.md`。

> **本包分发说明（README 补充）**
>
> 本包按 **“代码全量交付、数据按需重建”** 模式分发：为避免第三方数据集再分发
> 风险并控制包体积，动作数据集、部分官方权重与闭源 SDK 不随包提供。
>
> - **完整复原步骤**（数据下载、GMR 重定向、格式转换、metadata 重建与基线校验）：
>   见 **[RESTORE.md](RESTORE.md)**。
> - **需自行获取的第三方资产**：
>   - 动作数据（Bones-SEED，SMPL 版约 30 GB）：HuggingFace `nvidia/GEAR-SONIC`
>     / `bones-studio/seed` —— 门控下载 + 自定义许可，仅限许可证定义的合格
>     学术用户和合格初创企业；其他用途需联系发布方；
>   - 官方权重 `sonic_release/last.pt` —— NVIDIA Open Model License；
>   - Manus VR / PICO XR 遥操作 SDK —— 厂商闭源，需向厂商申请。
> - **校验基线**：`python tools/restore_motion_data.py --verify`
...
- **[2026-03-16]** 📦 [BONES-SEED](https://huggingface.co/datasets/bones-studio/seed) published under a gated custom license — 142K+ human motions (~288 hours) with G1 MuJoCo trajectories.
...
This distribution contains components under different licenses:

- **NVIDIA-origin source code**: Apache License 2.0, except where a component notice states otherwise
- **Teleoperation GUI**: AGPL-3.0-or-later under `decoupled_wbc/control/teleop/gui/`
- **RoboCasa-derived code and upstream assets**: MIT and CC BY 4.0 respectively; see the provenance limitation in the third-party notice
- **TienKung2 Pro robot description**: OpenAtom Open Hardware License Version 1.0
- **Model Weights**: Licensed under NVIDIA Open Model License - applies to all trained model checkpoints and weights

See [LICENSE](LICENSE), [NOTICE](NOTICE), and
[`legal/THIRD-PARTY SOFTWARE NOTICES AND ASSET LICENSES - GEAR-SONIC.txt`](legal/THIRD-PARTY%20SOFTWARE%20NOTICES%20AND%20ASSET%20LICENSES%20-%20GEAR-SONIC.txt)
for the controlling terms and component boundaries.

Training data includes Motion Data by Bones Studio with a link to
https://bones.studio/. Use of the underlying dataset is subject to the BONES
Motion Capture Dataset License Agreement.
...
All third-party trademarks and product names are the property of their respective
owners. Their use identifies relevant products or component origins and does not
imply endorsement.
...
For questions and issues about this downstream TienKung2 Pro package, use the
support or issue channel supplied by the distributor from which you received it.
The NVIDIA upstream contacts are not the support channel for downstream changes.
